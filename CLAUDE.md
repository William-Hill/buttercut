...
3. **Edit** → Use `roughcut` skill to create timeline scripts from transcripts
   ...
   - **Artifacts produced** (alongside the rough cut YAML): an editor XML (`*.xml`), an editorial recipe (`*.recipe.json`) capturing per-clip directives — speed ramps, color tags, markers, transitions, render preset, PowerGrade — and, for Resolve, a generated apply script (`*_apply.py`) that walks the recipe and applies what the Resolve scripting API supports (speed ramps, color tags, markers, render preset; PowerGrade best-effort). FCPXML output is now version 1.10 and emits `<timeMap>` natively for speed ramps, so Resolve can preserve ramps on import without running the apply script. An optional `*_edit_guide.html` documents the directives for human reference. Optionally, a b-roll manifest (`*.broll.yaml`, schema in `templates/broll_template.yaml`, validated by `lib/buttercut/broll_manifest.rb`) lists AI-generated graphics to be rendered via Hyperframes and placed on the timeline — see the Hyperframes integration epic for the full pipeline. Manifest entries are rendered to MP4 via the `render-broll` skill (one entry → one MP4 under `libraries/<library>/broll/<id>.mp4`); the parent updates each entry's `rendered` field after the sub-agent returns. **Late render**: the export step itself auto-renders any manifest entry whose `rendered` is empty or whose MP4 is missing on disk, then writes the populated path back to the manifest. This means iterating on the manifest YAML before export costs nothing — only `bundle exec ./.claude/skills/roughcut/export_to_fcpxml.rb …` triggers actual Hyperframes renders. Pass `--no-render` (or `BUTTERCUT_SKIP_BROLL_RENDER=1`) to bypass the late-render pass. Manifest entries with `rendered` populated are placed onto the timeline by the rough-cut export step (sibling-file convention `<roughcut>.broll.yaml`); each entry rides on V2 with muted audio so the V1 spine and audio bed are untouched.

   - **Swap-in-place loop** (watch in NLE → ask agent to fix → reload): once a roughcut is exported and imported into Resolve/FCP, the user spots a problem with one of the rendered b-roll clips ("the command at 0:42 should say `git rebase -i HEAD~3`, not `get rebase`"). The agent edits the offending entry's `content` in `<roughcut>.broll.yaml`, then **invokes the `render-broll` skill on just that entry**. Because `BrollRenderer` is idempotent on the same `<id>` — it deletes and rewrites `libraries/<library>/broll/<id>.mp4` — the timing, recipe, and XML never change. The user reloads the project in their NLE and the new MP4 is picked up automatically. No re-export needed.
