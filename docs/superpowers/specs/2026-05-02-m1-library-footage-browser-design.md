...
- On Library window open, frontend calls a new Tauri command `allow_video_paths(root: String)` that calls `app.asset_protocol_scope().allow_directory(root, recursive=true)`. Scope is additive across libraries — opening a second library extends the allow-list, doesn't replace it. The command validates that `root` matches the `video_paths_root` returned by a recent `get_library` call (to prevent the frontend from arbitrarily widening scope).
