...
### Added
- **Code-callout normalization for verbal-form leakage (#32).** The b-roll director now translates spoken transcript forms ("get rebase dash i tilde three") into canonical syntax (`git rebase -i HEAD~3`) for `code-callout` candidates, cross-checks against the on-screen visual transcript at the same timestamp, and prefers the on-screen form when the two disagree (logging the discrepancy in the entry's `notes`). A new `code_vocabulary: []` field under `library.yaml` `broll:` lets each library declare CLI tools and language keywords the director can trust as code (e.g. `[git, npm, kubectl]`). A deterministic postprocess validator drops any `code-callout` candidate whose `command` still looks like prose — better to render nothing than confidently wrong code. Migration: `ruby scripts/006_migrate_add_code_vocabulary.rb --all` (requires 005 to have run first).
- **B-roll density controls + score threshold + per-library blacklist (#31).** New `broll:` block in `library.yaml` with `density` (`low`/`medium`/`high` → 2/4/8 graphics per minute), `score_threshold` (drops candidates below this floor), and `blacklist_terms` (terms that should never become a graphic — case-insensitive substring match against candidate `content`). The b-roll director now resolves these in priority order: explicit caller param → library `broll:` block → module default. Migration: `ruby scripts/005_migrate_add_broll.rb --all` adds the default block to libraries that predate the feature.
